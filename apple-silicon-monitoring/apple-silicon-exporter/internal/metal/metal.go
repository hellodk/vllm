// Package metal provides GPU metrics via Metal Performance Shaders
// This uses Objective-C via cgo to access Metal APIs
package metal
...
func (c *Collector) Collect() (*Metrics, error) {
	...
	if result := C.get_metal_gpu_info(&gpuInfo); result == 0 {
		...
		// Try to get utilization from counter sampling
